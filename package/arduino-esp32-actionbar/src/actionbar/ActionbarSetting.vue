<template>
    <div>
        <v-tooltip bottom>
            ...
            <span>Kart Ayarları</span>
        </v-tooltip>

        <v-dialog v-model="settingDialog" max-width="500px">
            <v-card>
                <v-card-title>
                    <span class="headline">Kart İletişim Ayarları</span>
                </v-card-title>
                <v-divider></v-divider>
                <v-card-text>
                    <v-container grid-list-md>
                        <v-layout wrap>
                            <v-flex xs12>
                                <v-subheader class="pa-0 mb-2">Kart İletişim Ayarları</v-subheader>
                                <div class="d-flex">
                                    <v-combobox
                                            class="mr-3"
                                            v-model="currentPort"
                                            :items="comports"
                                            label="İletişim kanalı (COM PORT)"
                                    ></v-combobox>
                                    <v-combobox
                                            v-model="baudrate"
                                            :items="baudrates"
                                            label="Seri iletişim hızı"
                                    ></v-combobox>
                                </div>
                            </v-flex>
                        </v-layout>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" flat @click.native="settingDialog = false">Kapat</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
