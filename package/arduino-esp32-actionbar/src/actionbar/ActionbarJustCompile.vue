<template>
    <div>
        ...
        <v-dialog v-model="compileDialog" persistent max-width="450px">
            <v-card>
                <v-card-title>
                    <span class="headline">Derleme Durumu</span>
                </v-card-title>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="blue darken-1" flat @click="run"
                           :disabled="compileStep <= 2 && failed === false">Yeniden Derle
                    </v-btn>
                    <v-btn color="blue darken-1" flat @click="compileDialog = false"
                           :disabled="compileStep <= 2 && failed === false">Kapat
                    </v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    ...
    methods: {
      ...
      run() { //find port and mac
        ...
        p.then(boardMac => {
          ...
          console.log("---> step 2 <---");
          this.stepResult["2"].msg = "Derleniyor...";
