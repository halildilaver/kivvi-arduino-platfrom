<template>
    <div>
        <v-tooltip bottom>
            ...
            <span>Derle &#x26; Yükle</span>
        </v-tooltip>
        <v-dialog v-model="compileDialog" persistent max-width="450px">
            <v-card>
                <v-card-title>
                    <span class="headline">Derle &#x26; Yükle</span>
                </v-card-title>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="blue darken-1" flat @click="compileDialog = false"
                           :disabled="compileStep < 4 && failed === false">Kapat
                    </v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
  ...
  export default {
    ...
    methods: {
      run() { //find port and mac
        ...
        this.stepResult["1"].msg = `Kart ${comport} portunu kullanarak bulunuyor`;
        if (!comport) {
        ...
        boardCompiler.readMac({
          ...
          this.stepResult["2"].msg = "Kod derleniyor...";
          //setInterval(() => {
          //console.log("running..");
          ...
          this.stepResult["2"].msg = "Derleme tamamlandı!";
          this.compileStep = 3;
          this.stepResult["3"].msg = "Yükleniyor ... ";
          console.log("---> step 3 <---");
          G.$emit("upload-begin"); //<<<<< fire event
          return boardCompiler.flash(comport, baudrate);
        }).then(() => {
          this.stepResult["3"].msg = "Yükleme Başarılı";
          this.compileStep = 4;
